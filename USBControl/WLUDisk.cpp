...
vector<wstring> CWLUDisk::m_stcDevInfo = {_T("cdrom"), _T("net"), _T("bluetooth"), _T("ports"), _T("diskdrive"), _T("floppydisk"), _T("wpd"), _T("usb")};
...
* @date         2023-10-10
*/
BOOL CWLUDisk::GetUsbDeviceDeviceName(const WCHAR* pszUSBHubDevicePath, int ConnectIndex, __out std::wstring& wstrUSBDeviceName, std::wstring& strError) {
    DWORD   dwError = 0;
    wstring strTemp;
    BOOL    bResult = false;
    wstring cstr;
...
BOOL CWLUDisk::HubGetNodeStringDescriptorProduct(_In_ HANDLE hHubDevice, _In_ ULONG ConnectIndex, _In_ USB_DEVICE_DESCRIPTOR& DevDescInfo, __out wstring& WstrDesc, __out std::wstring& strError) {
...
BOOL CWLUDisk::GetStringDescriptor(_In_ HANDLE hHubDevice, ULONG ConnectionIndex, UCHAR DescriptorIndex, USHORT LanguageID, __out wstring& WstrOutDesc, __out std::wstring& strError) {
    BOOL    bRet           = FALSE;
    BOOL    success        = 0;
    ULONG   nBytes         = 0;
...
* @date         2023-10-10
*/
BOOL CWLUDisk::HubGetNodeConnectionInformation(HANDLE hHubDevice, int ConnectIndex, USB_NODE_CONNECTION_INFORMATION& connectinfo, std::wstring& strError) {
    BOOL    bRet   = FALSE;
    ...
    BOOL success = DeviceIoControl(hHubDevice, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION, pConnectionInfo, nBytes, pConnectionInfo, nBytes, &nBytes, NULL);
...
BOOL CWLUDisk::GetDeviceFullInfo(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDeviceInfoData, vector<boost::shared_ptr<DeviceInfoFull>>& VectAllDevice, vector<DevPathAndDevInst>& vectHubPathAndDevinst) {
    ...
    SetupDiGetDeviceRegistryProperty(hDevInfo, pDeviceInfoData, SPDRP_CLASS, NULL, (PBYTE)wszClassName, sizeof(wszClassName), NULL);
...
void CWLUDisk::DealDeviceChangeMsg() {
    ...
    */
    ...
    for (it = g_vecDevice.begin(); it != g_vecDevice.end(); ++it) {
        ...
        if (DeviceInfo.isParentUSB) {
            WriteInfo(("VID: {} PID: {}  ProductInfo: {}  VendorInfo: {}"), DeviceInfo.UsbVid, DeviceInfo.UsbPid, CStrUtil::UnicodeToUTF8(DeviceInfo.wsProductInfo),
    ...
    if (uMsg == WM_DEVICECHANGE) {
        ...
        if (DBT_DEVICEARRIVAL == wParam || DBT_DEVICEREMOVECOMPLETE == wParam) {
            ...
            if (pHdr->dbch_devicetype == DBT_DEVTYP_VOLUME) {
                ...
                WriteInfo(("Volume changed,dbcv_size: {} dbcv_devicetype: {} dbcv_reserved: {}  dbcv_unitmask: {}  dbcv_flags: {}"), pVolume->dbcv_size, pVolume->dbcv_devicetype,
                          pVolume->dbcv_reserved, pVolume->dbcv_unitmask, pVolume->dbcv_flags);
