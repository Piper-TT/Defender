...
BOOL CWindowsHelper::GetMacByIp(__in const wstring wsIp, __out wstring& wstrMacAddress) {
    ...
    if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
        ...
        while (pAdapter) {
            if (pAdapter->Type == MIB_IF_TYPE_ETHERNET || pAdapter->Type == 71  // pAdapter->Type是71为：无线网卡
            ) {
...
BOOL CWindowsHelper::QueryServiceInfoByName(const tstring& strSrvName, DWORD& dwCurrentState, DWORD& dwStartType, tstring* pStrErr,
                                            DWORD* pLastErrCode, tstring* pStrServiceDisplayName) {
    BOOL                bRes = FALSE;
...
Wow64RedirectOff::Wow64RedirectOff() {
    ...
    LPFN_Disable = (FN_Wow64DisableWow64FsRedirection)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "Wow64DisableWow64FsRedirection");
    if (LPFN_Disable) {
...
VOID Wow64RedirectOff::SetWow64RedirectOn() {
    ...
    if (LPFN_Disable) {
        FN_Wow64RevertWow64FsRedirection LPFN_Revert =
            (FN_Wow64RevertWow64FsRedirection)GetProcAddress(GetModuleHandle(TEXT("kernel32")), "Wow64RevertWow64FsRedirection");
        if (LPFN_Revert) {
