...
struct WriteBufferIo {
    uint8_t* buf;
    uint32_t pos, size;
    WriteBufferIo() : buf(NULL), pos(0), size(0) {}
...
uint8_t* ClipboardManage::FromBitMap(const BITMAPINFO& bmp_info, const void* bits, long& size) {
    ...
    // TODO detect gray
    png_set_IHDR(png, info, head.biWidth, head.biHeight, out_bits > 8 ? 8 : out_bits, color_type, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
