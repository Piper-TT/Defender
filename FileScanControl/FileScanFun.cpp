...
// 线程池扫描任务包装函数
void ScanDriveTask(std::wstring wstrDrive, PECacheHelper* pCacheHelper) {
    CFileScanFun FileScan;
    FileScan.m_PeCacheHelper = pCacheHelper;
    FileScan.GetFileListByFolder(wstrDrive);
}
...
BOOL CFileScanFun::EnableScanFileFunction() {
    ...
    WriteInfo("Starting file scan on all drives...");
    ...
    for (TCHAR letter = 'A'; letter <= 'Z'; ++letter) {
        ...
        if ((drives & 1) == 1) {
            wchar_t DrivePath[MAX_PATH] = {0};
            swprintf_s(DrivePath, MAX_PATH, L"%c:\\", letter);
            
            std::wstring wstrDrive(DrivePath);
            WriteInfo("Scanning drive: {}", CStrUtil::ConvertW2A(wstrDrive).c_str());
            
            // 提交扫描任务到线程池
            Pool.submit(ScanDriveTask, wstrDrive, m_PeCacheHelper);
        }
        drives >>= 1;
    }
...
BOOL CFileScanFun::GetFileListByFolder(const std::wstring wstrFolder) {
    ...
    2.硬链接 （只能是文件）就是普通文件，无法与真身区分，或者说每个都是真身 类型由内核维护
    3.软链接 （只能是目录）通过文件属性FILE_ATTRIBUTE_REPARSE_POINT识别，暂不知如何与符号链接区分 类型由内核维护
    ...
    */
    ...
    do {
        ...
        if (finder.attrib & _A_SUBDIR) {  // 如果是目录则递归
            ...
            if (-1 != strCurFolder.find(_T("recycler")) || -1 != strCurFolder.find(_T(".bin"))) {
                continue;
            }

            // skip system temp folder
            ...
            if (CheckIsPEFile(wstrFullFileName)) {
                ...
                if (m_PeCacheHelper && !m_PeCacheHelper->PE_CACHE_Query_Cache(wstrFullFileName, FileSize, LastWriteTime, NULL, NULL, NULL)) {
...
BOOL CFileScanFun::CheckIsPEFile(const std::wstring wstrFilePath) {
    ...
    if (hFileHandle == INVALID_HANDLE_VALUE) {
        WriteError("Failed to open file  GetLastError = {} filepath = {}", GetLastError(), CStrUtil::ConvertW2A(wstrFilePath).c_str());
        goto END;
    }

    // 读取文件的前两个字节
    if (!ReadFile(hFileHandle, buffer, sizeof(buffer), &bytesRead, NULL) || bytesRead != sizeof(buffer)) {
        WriteError("Failed to ReadFile GetLastError = {} filepath = {} ", GetLastError(), CStrUtil::ConvertW2A(wstrFilePath).c_str());
        goto END;
    }
