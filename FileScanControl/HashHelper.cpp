...
unsigned char* __PeCacheValue(LIST_HEAD* entry, UCHAR type) {
...
BOOL __PeCacheRelease(LIST_HEAD* entry, BOOL allref, UCHAR type) {
...
static unsigned int hashtb_hash(HASHTB* table, unsigned int* valPtr) {
    if (!table->hash_calc) {
        return 0;
    } else {
...
static BOOLEAN __hashtb_equal(HASHTB* table, LIST_HEAD* item, unsigned int* ptrVal, UCHAR type) {
...
static BOOLEAN __hashtb_insert_item(HASHTB* table, LIST_HEAD* item, unsigned int* valPtr, UCHAR type) {
    unsigned int index;
    LIST_HEAD*   entry;
...
BOOLEAN
hashtb_insert_item(HASHTB* table, LIST_HEAD* item, unsigned int* valPtr, UCHAR type) {
    BOOL retval = FALSE;
...
LIST_HEAD* hashtb_query_item(HASHTB* table, unsigned int* valPtr, UCHAR type) {
    unsigned int index;
    LIST_HEAD*   entry;
    if ((0 == table->bucket_count)) {
        return NULL;
...
void hashtb_remove_all(HASHTB* table, UCHAR type) {
    if ((0 == table->bucket_count)) {
        return;
    }
